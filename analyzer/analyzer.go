...
func (a *analyzer) Analyze(chat *chat.Chat) (*report.ChatAnalytics, error) {
	...
	for scanner.Scan() {
		...
		// Parse line
		date, err := a.parser.Date(line)
		if err != nil {
			return nil, err
		}
		hour, err := a.parser.Hour(line)
		if err != nil {
			return nil, err
		}
		message, err := a.parser.Message(line)
		if err != nil {
			return nil, err
		}
		sender, err := a.parser.Sender(line)
		if err != nil {
			return nil, err
		}
...
func cleanMessage(r rune) rune {
	switch {
	case r == '\'' || r == '’' || r == ',' || r == '.' || r == '!' || r == '"' || r == '>' || r == '<' || r == '?' || r == '(' || r == ')' || r == ':' || unicode.IsDigit(r):
		return -1
	}
	return r
}
