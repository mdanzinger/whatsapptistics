...
// Poll satisifies the ReportPoller interface by polling sqs for messages and returning a slice of report from their ids
...
// NewReportPoller returns a new sqs reportPoller that can be polled to receive new report.
func NewReportPoller() *reportPoller {
	...
	resultURL, err := s.GetQueueUrl(&sqs.GetQueueUrlInput{
		QueueName: aws.String("whatsapp-chat"),
	})
