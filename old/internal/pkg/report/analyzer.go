...
func (ra *reportAnalyzerService) Start() {
	...
	for {
		...
		// We have some report to process!
		if len(reports) > 0 {
			ra.process(reports) //
		}
	}
...
func (ra *reportAnalyzerService) process(reports []Report) {
	log.Println("Received %d report", len(reports))
	// Create waitgroup. We don't want processReports to continue receiving report until we've processed all report given.
